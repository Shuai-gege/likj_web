<template>
  <div class="detail">
    <navbar title="下级提现记录"></navbar>
    <div class="null" v-if="list.length==0" style="background-color: #fff;">暂无下级提现记录</div>
    <mescroll-vue ref="mescroll" :down="mescrollDown" :up="mescrollUp" @init="mescrollInit">
      <div class="box">
        <div class="flex minxi" v-for="(item,i) in list" :key="i" @click="todetail(item.cash_id)">
          <div class="flex">
            <div>
              <img :src="item.user.wx_avatar" alt />
            </div>
            <div>
              <p
...
export default {
  ...
  methods: {
    ...
    upCallback(page, mescroll) {
      ...
      this.axios
        .post("/api/property_administer/lowerCashList", {
          page: page.num
        })
        .then(data => {
          // this.list = data;
